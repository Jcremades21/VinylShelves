const Reporte = require('../models/reporte_comentario');
const validator = require('validator');
const { validationResult } = require('express-validator');
const { infoToken } = require('../helpers/infotoken');

const obtenerReporteC = async(req, res) => {
    ...
    try {
        ...
        // Si no ha llegado ID, hacemos el get / paginado
                ...
                if(pag){
                [reportesc, total] = await Promise.all([
                    Reporte.find({}).skip(desde).populate('usuario', '-__v').populate('usuario_reportado', '-__v').populate('comentario', '-__v'),
                    Reporte.countDocuments()
                ]);
                }
                else{
                [reportesc, total] = await Promise.all([
                    Reporte.find({}).skip(desde).limit(registropp).populate('usuario', '-__v').populate('usuario_reportado', '-__v').populate('comentario', '-__v'),
                    Reporte.countDocuments()
                ]);
                }
...
const crearReportesC = async(req, res) => {
...
const actualizarReportesC = async(req, res) => {
...
const borrarReportesC = async(req, res) => {
...
module.exports = { obtenerReporteC, crearReportesC, actualizarReportesC, borrarReportesC }