...
export default function UserReviews({ navigation, route }) {
    ...
    React.useEffect(() => {
        ...
        //console.log(ret.useremail);
            ...
            axios.get(url,
              ...
              console.log(res.data.usuarios.reviews);
            })
            .catch((error) => {
              console.error(error)
                });
    ...
    return (
        <View style={styles.Fondo}>
            ...
            {usu ? <PageHeader>{usu.username}'s reviews</PageHeader>:null}
            </View>
            ...
            renderItem={({ item }) => (
                ...
                <TouchableOpacity onPress={() => navigation.navigate('ReviewScreen', { id: item._id })}>
                     <View style={styles.infoDiv}>
                    <Image source={{ uri: item.albumimg }} style={styles.image2}  />
                     <View  style={styles.restitle2}>
                     <View style={styles.icondiv}>                                              
                     <Text numberOfLines={1} style={styles.itemText}>{item.titulo}</Text>
                      <FontAwesome5 name="newspaper" size={24} color="white" />
                    </View>
                <Text numberOfLines={2} style={styles.itemText2}>{item.texto}</Text>
                <View style={styles.icons}><AntDesign name="hearto" size={20} color="#B81EFF" /><Text style={styles.like}> {item.likes.length}</Text></View>
                </View>
                </View>
                </TouchableOpacity>
                </View></>
